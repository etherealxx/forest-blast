...
#	nametag.reparent(get_parent())
#	nametag.parent_bind = self
...
#	animation_tree.set("parameters/Move/blend_position", animated_velocity.move_toward(relative_velocity, delta / 0.2))
	...
	if movement != Vector3.ZERO:
		animation_tree.set("parameters/Run/blend_amount", lerp(animation_tree.get("parameters/Run/blend_amount"), 1.0, delta * 14.0))
	else:
		animation_tree.set("parameters/Run/blend_amount", lerp(animation_tree.get("parameters/Run/blend_amount"), 0.0, delta * 14.0))
	# Float
